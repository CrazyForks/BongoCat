...
部分游戏在某些系统上仍会出现后台输入中断。1.4.3 改进了输入处理和诊断，但尚未解决所有已反馈的游戏兼容性问题。实现边界和当前限制见 [Windows 输入与游戏兼容性说明](../SECURITY.md#windows-input-and-game-compatibility)。使用官方接口也不能保证所有反作弊产品都不会误判。
