...
void bongo_cat_app_refresh_texture_resolution(BongoCatApp *app) {
    if (!app || !app->window || !app->gl_context) return;
    ...
    bool active = app->session.window.visible && !app->window_minimized &&
        ...
        !app->window_snapshot;
    if (!bongo_cat_live2d_texture_refresh_pending(app->live2d, active)) return;
    ...
    if (bongo_cat_live2d_refresh_textures(app->live2d, active)) {
        app->dirty = true;
