...
public:
    ...
    /* A cancelled replacement may have left driver-side storage retiring.
       Hold only a follow-up replacement during that short observation window;
       successful atlas commits can continue without an artificial pause. */
    bool cancellation_cooldown(uint64_t now) const {
        return cancelled_ && sample_ns_ && now < sample_ns_;
    }
