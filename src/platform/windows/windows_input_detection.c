...
bool bongo_cat_windows_pointer_detect(WindowsPointerDetection *state,
    ...
    if (hint != WINDOWS_POINTER_DESKTOP) {
        if (!state->hint_pending) {
            state->hint_pending = true;
            state->hint_ms = now_ms;
        }
        /* Hidden-on-typing cursors alone are not evidence of mouse movement. */
    ...
    if (window_age >= 128 && (state->moving_samples >= 4 || window_age >= 256)) {
        ...
        if (hint == WINDOWS_POINTER_DESKTOP && (stalled || recentered)) {
            state->relative = true;
            state->reason = stalled ? WINDOWS_POINTER_STALLED :
                WINDOWS_POINTER_RECENTERED;
            state->evidence_ms = now_ms;
        } else if (hint == WINDOWS_POINTER_DESKTOP && state->relative &&
            now_ms - state->evidence_ms >= 192 &&
            (distance > 2 || state->reason == WINDOWS_POINTER_HIDDEN ||
                state->reason == WINDOWS_POINTER_CLIPPED)) {
